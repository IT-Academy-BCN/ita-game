...
const Admin = () => {
  ...
  const [startDate, setStartDate] = useState(null);
   ...
   useEffect(() => {
   const newData = { selectedStack, selectedUser, selectedActivity, startDate };
   setData(newData);
      ...
      <div className="container min-w-full flex flex-col bg-slate-100 pb-10">
          ...
          <div className="flex flex-row">
            <div className="avatar">
              <div className="w-14 rounded-full px-2 py-2">
                <img
                  src={import.meta.resolve(`../assets/images/${selectedStack ? selectedStack.stack : "angular"}.png`)}
                  alt="Angular logo"
                />
              </div>
            </div>
            <div className="flex flex-col justify-center pl-2 my-1">
              <h2 className="font-bold text-black">{selectedStack ? selectedStack.stack : "Categoria"}</h2>
            </div>
          </div>
          <div>
        ...
        >
          <div className="flex flex-row">
            ...
            <div className="flex flex-col justify-center pl-2 my-1">
              <h2 className="font-bold text-black">{selectedUser ? selectedUser.name : "Alumno"}</h2>
            </div>
          </div>
          <div>
            <img className="w-6" src={arrowDown} alt="Arrow down" />
        ...
        >
          <div className="flex flex-row">
            <div className="avatar">
              <div className="w-14 rounded-full px-2 py-2">
                 <img src={selectedActivity ? selectedActivity.img : thumbUp} alt={`Logo actividad`} />
              </div>
            </div>
            <div className="flex flex-col justify-center pl-2 my-1">
              <h2 className="font-bold text-black">{selectedActivity ? selectedActivity.type : "Actividad"}</h2>
            </div>
          </div>
          <div>
            <img className="w-6" src={arrowDown} alt="Arrow down" />
        ...
        <div className="flex justify-center mb-3">
          <button type="submit" className="btn  btn-primary mt-6 w-80 px-16">
            <span onClick={handleSubmit} className="text-gray-900">Enviar</span>
          </button>
        </div>
